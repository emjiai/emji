...
const MindMapView: React.FC<MindMapViewProps> = ({ data }) => {
  ...
  // Fit to screen (public method)
  const handleFitToScreen = () => {
    handleFitToScreenInternal();
  };

  // Internal fit to screen function that can accept positions parameter
  const handleFitToScreenInternal = (positions: Record<string, { x: number, y: number }> = nodePositions) => {
  ...
  useEffect(() => {
    ...
    // Calculate initial positions in a tree layout with increased distances
    ...
    // Position level 2 nodes in a circle around root with increased radius
    ...
    level2Nodes.forEach((node, index) => {
      const radius = 350; // Increased from 250 to 350
    ...
    // For each level 2 node, position its level 3 children with increased distance
    level2Nodes.forEach((parentNode, parentIndex) => {
      ...
      if (childNodes.length > 0) {
        ...
        childNodes.forEach((childNode, childIndex) => {
          const radius = 600; // Increased from 450 to 600
    ...
    // Auto-fit to screen after positioning nodes
    setTimeout(() => {
      handleFitToScreenInternal(newPositions);
    }, 100);
  }, [data]);
