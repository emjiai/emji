...
const ContentDisplay = ({
  ...
  const [readerData, setReaderData] = useState(propReaderData);
  const [editedContent, setEditedContent] = useState<any>(null);
  const [pdfUrl, setPdfUrl] = useState<string>('');

  // Update state when props change
  React.useEffect(() => {
    setReaderData(propReaderData);
    setEditedContent(propReaderData);
  }, [propReaderData]);

  const handleCopyCitation = (index: number) => {
    if (readerData.citations && readerData.citations[index]) {
      const citation = readerData.citations[index];
  ...
  const renderDocumentViewer = () => (
    <div className="flex flex-col h-full">
      <div className="flex-1 h-full overflow-auto">
        {/* Replace ContentView with EditContentView */}
        <EditContentView 
          content={editedContent} 
          onContentUpdate={(section, path, newContent) => {
            // Here you can handle updates to specific sections
