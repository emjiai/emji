...
// Import Components
import ChatInterface from "../_components/ChatInterface"; // For text chat mode
import TalkInterface from "../_components/TalkInterface"; // For voice chat mode
import { Button } from "@/components/ui/button";

// Import utility functions
import { getVoiceOptions } from '@/utils/voice-utils'; // Assuming still needed for potential future use
...
export default function TutorPage() {
  ...
  const [loading, setLoading] = useState(true); // Loading state for tutor data
  const [error, setError] = useState<string | null>(null);
  const [chatMode, setChatMode] = useState<'text' | 'voice'>('text'); // Explicitly define possible modes

  // Chat state
  const [messages, setMessages] = useState<Message[]>([]);
  const [chatLoading, setChatLoading] = useState(false); // Loading state for AI message response

  // Removed unused voiceOptions state as it's not passed to TalkInterface currently
  // const [voiceOptions, setVoiceOptions] = useState<{ ... } | null>(null);
  // Removed unused document state
  ...
  // Function to send a message (used by both ChatInterface and TalkInterface)
  const sendMessage = async (content: string) => {
    ...
    try {
      ...
      // Prepare messages for API (simplified for this example)
      // You might want to include previous messages for context in a real app
      const apiMessages = [
        {
          role: "system", // Example: Define system prompt separately
      ...
      const response = await fetch(`${API_BASE_URL}/api/v1/chat`, {
        ...
        credentials: 'include',
        body: JSON.stringify({
          messages: apiMessages, // Send formatted messages
